...
class DurableStore:
    ...
    def enqueue_agent_session_controls(
        self,
        *,
        operation_id: str,
        agent_id: str,
        ...
        """Enqueue the controller-owned session panel for an active turn."""
        timestamp = time.time() if now is None else float(now)
        self.connection.execute("BEGIN IMMEDIATE")
        try:
            target = self.agent_notification_target(
            ...
            binding = self.resolve_surface_binding(
                target.chat_id,
                thread_id,
            )
            if (
                ...
                or binding.target_id != str(agent_id)
            ):
                raise StoreError(
                    "Session controls can only be shown in the managed "
                    "agent's own Telegram topic."
                )
            row = self.connection.execute(
                """
                SELECT m.source_inbox_job_id, a.*
                FROM agent_mailbox AS m
                JOIN agents AS a ON a.agent_id = m.agent_id
                WHERE m.mailbox_id = ? AND m.agent_id = ?
                    AND m.state = 'leased' AND m.lease_owner = ?
            ...
            if row is None:
                raise LeaseLostError(
                    "Session controls are only available to the active "
                    "managed turn."
                )
            authorized_user_id = self._authorized_user_for_inbox_job(
                int(row["source_inbox_job_id"])
            ...
            if authorized_user_id is None:
                raise StoreError(
                    "The active turn has no authorized Telegram user."
                )
            agent = self._managed_agent_from_row(row)
            prefix = f"agent-mailbox:{int(mailbox_id)}:session-controls"
            lifecycle_type = (
                "agent_resume"
            ...
            lifecycle = self.create_callback_action(
                operation_id=f"{prefix}:{lifecycle_type}",
                action_type=lifecycle_type,
            ...
            fresh = self.create_callback_action(
                operation_id=f"{prefix}:new-session",
            ...
            rows = [
                [
                    {
                        "text": lifecycle_label,
                        "callback_data": f"a:{lifecycle.token}",
                    },
                    {
                        "text": "New session…",
                        "callback_data": f"a:{fresh.token}",
                    },
                ]
            ]
            if agent.provider == "codex":
                resume = self.create_callback_action(
                    operation_id=f"{prefix}:resume-session",
                    ...
                    [
                        {
                            ...
                            "callback_data": f"a:{resume.token}",
                        }
                    ]
                )
            session = (
                "persisted"
                if agent.provider_session_id is not None
                else "not started"
            )
            text = (
                "🎛 Session controls\n\n"
                f"Agent: {target.speaker}\n"
                f"Provider: {agent.provider}\n"
                f"Session: {session}\n\n"
                "Session changes apply after the current turn finishes."
            )
            message_id = self.enqueue_api_call(
                operation_id=operation_id,
                method="sendMessage",
