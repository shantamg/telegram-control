...
def build_main_agent_prompt(
    ...
    return (
        ...
        "A project can live either as a topic in this private chat "
        "(create_project_agent) or as its own Telegram group. Telegram forbids "
        "bots from creating a group, adding themselves, or enabling Topics, so "
        "when the user wants a separate group, respond telling them to send "
        "/newgroup here: that replies with a one-tap link which adds this bot "
        "to a group they pick with the rights it needs. Do not propose a "
        "private-chat topic as if it were the group they asked for.\n\n"
